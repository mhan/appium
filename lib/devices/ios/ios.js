    ...
    _(contexts).each(function (ctx) {
      if (ctx.view && (ctx.view.title || "").match(titleRegex)) {
        if (ctx.view.url === "about:blank") {
          ...
          // or in the case of iOS 7.1 in a webview (not in Safari)
          // or in the case of Safari on a device + iOS 8.0+
          ...
          if (parseFloat(this.iOSSDKVersion) < 7 ||
              ...
              (parseFloat(this.args.platformVersion) >= 8.0 && this.args.udid)) {
            matchingCtx = ctx;
          }
        } else {
          matchingCtx = ctx;
